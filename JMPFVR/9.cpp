...
//             if (vertex_set[j] == 0){ 
//                 vertex_set[j] = -1; 
//                 adj[j].push_back(prufer[i] - 1); 
//                 adj[prufer[i] - 1].push_back(j);
//                 // pii(j + 1, prufer[i]), nl; 
...
void bfs(int m, int dg){
    int i, j, cnt = 0, sz, d; 
    queue <int> q; 
    q.push(0), ++cnt; if(cnt == m) return; 
    while(!q.empty()){
        int u = q.front(); q.pop(); 
        sz = q.size(); 
        if(sz == 0) d = rnd(dg) + 1; 
        ...
        for0(i, d){
            // pii(u, d); nl; 
            adj[u].push_back(cnt), adj[cnt].push_back(u); 
            q.push(cnt); cnt++; 
            if(cnt == m) return; 
        } 
    }
