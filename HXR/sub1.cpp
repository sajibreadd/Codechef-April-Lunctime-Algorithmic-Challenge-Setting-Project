...
int n, k, l[N + 5], r[N + 5]; 
ll ara[N + 5], sol[N + 5]; 
int z[N + 5][N + 5], res[N + 5][N + 5];  
void mat_mul(int a[][N + 5], int b[][N + 5]){
	int i, j, c[N + 5][N + 5], p;
	for0(i, n){
		for0(j, n){
			for(p = c[i][j] = 0; p < n; ++p) c[i][j] = add(c[i][j], mul(a[i][p], b[p][j])); 
		}
	} 
	for0(i, n) for0(j, n) a[i][j] = c[i][j]; 
}
void expo(int p){
	int i, j;
	for0(i, n) res[i][i] = 1; 
	while(p){
		if(p & 1) mat_mul(res, z); 
		mat_mul(z, z), p >>= 1; 
	}
}
void solve(){
	int i, j, p; 
    mem(z, 0), mem(res, 0), mem(sol, 0); 
	for0(i, n) for(j = l[i]; j <= r[i]; ++j) z[i][j] = 1; 
	expo(k - 1); 
	int bit[N + 5]; 
	for0(i, 60){
		for0(j, n){
		...
		for0(j, n){
			int x = 0; 
			for0(p, n) x = add(x, mul(res[j][p], bit[p])); 
			if(x & 1) sol[j] |= 1ll << i; 
		}
	}
	ll xr = 0; 
...
int main(){
    ...
    int i, j;
        sii(n, k); 
        // assert(n >= 1 && n <= 100 && k >= 1 && k <= 50000000); 
        for0(i, n){
        	sl(ara[i]); 
